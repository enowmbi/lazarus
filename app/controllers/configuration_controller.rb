...
class ConfigurationController < ApplicationController
  ...
  def settings
    @config = Config.get_multiple_configs_as_hash ['InstitutionName', 'InstitutionAddress', 'InstitutionPhoneNo', \
    ...
    @enabled_grading_types = Config.get_grading_types
    @time_zones = TimeZone.all
    @school_detail = SchoolDetail.first || SchoolDetail.new
    @countries=Country.all
    if request.post?
      Config.set_config_values(params[:configuration])
      ...
      Config.clear_school_cache(@current_user)
