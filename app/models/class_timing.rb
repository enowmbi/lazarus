...
class ClassTiming < ApplicationRecord
  ...
  scope :for_batch, lambda { |b| where({:batch_id => b.to_i, :is_deleted=>false, :is_break => false}).order('start_time ASC')}
  default_scope lambda{where({ :batch_id => nil, :is_break => false, :is_deleted=>false}).order('start_time ASC')}
  scope :active_for_batch, lambda { |b| where({:batch_id => b.to_i, :is_deleted=>false}).order('start_time ASC') }
  scope :active, lambda{where({ :batch_id => nil, :is_deleted=>false}).order('start_time ASC')}
