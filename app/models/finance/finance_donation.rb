...
class FinanceDonation < ApplicationRecord
  belongs_to :transaction_entry, :class_name => 'FinanceTransaction',  :dependent => :destroy
  ...
  def create_finance_transaction
    transaction_entry = FinanceTransaction.create(
    ...
    self.transaction_id = transaction_entry.id
  end
end
