...
class FinanceTransaction < ApplicationRecord
  ...
  def self.report(start_date,end_date,page)
    ...
    self.where(["transaction_date >= ? AND transaction_date <= ? AND category_id NOT IN (#{fixed_cat_ids.join(",")})",start_date,end_date]).order('transaction_date')
  end
  ...
  def self.grand_total(start_date,end_date)
    ...
    fixed_transactions = FinanceTransaction.where(["transaction_date >= ? AND transaction_date <= ? AND category_id IN (#{fixed_cat_ids.join(",")})",start_date,end_date])
    other_transactions = FinanceTransaction.where(["transaction_date >= ? AND transaction_date <= ? AND category_id NOT IN (#{fixed_cat_ids.join(",")})",start_date,end_date])
  ...
  def self.total_fees(start_date,end_date)
    fee_id = FinanceTransactionCategory.find_by_name("Fee").id
    fees = 0
    fees = FinanceTransaction.find(["transaction_date >= ? AND transaction_date <= ? AND category_id = ?",start_date.to_date,end_date.to_date,fee_id]).map{|ft| ft.amount}.sum
  end
  ...
  def self.total_other_trans(start_date,end_date)
    ...
    fixed_cat_ids = FinanceTransactionCategory.where({:name=>cat_names}).collect(&:id)
    fees = 0
    transactions = FinanceTransaction.where(["created_at >= ? AND created_at <= ? AND category_id NOT IN (#{fixed_cat_ids.join(",")})",start_date,end_date])
  ...
  def self.donations_triggers(start_date,end_date)
    ...
    donations = FinanceTransaction.where(["transaction_date >= ? AND transaction_date <= ? AND master_transaction_id = 0 AND category_id = ?",start_date,end_date,donation_id])
    trigger = FinanceTransaction.where(["transaction_date >= ? AND transaction_date <= ? AND master_transaction_id != 0 and category_id = ?",start_date,end_date,donation_id])
    donations.each do |d|
  ...
  def self.expenses(start_date,end_date)
    expenses = FinanceTransaction.select('finance_transactions.*').joins('INNER JOIN finance_transaction_categories ON finance_transaction_categories.id = finance_transactions.category_id').where(["finance_transaction_categories.is_income = 0 and finance_transaction_categories.id != 1 and transaction_date >= ? AND transaction_date <= ?",start_date,end_date])
    expenses
  end

  def self.incomes(start_date,end_date)
    incomes = FinanceTransaction.select('finance_transactions.*').joins('INNER JOIN finance_transaction_categories ON finance_transaction_categories.id = finance_transactions.category_id').where(["finance_transaction_categories.is_income = 1 and transaction_date >= ? AND transaction_date <= ?",start_date,end_date])
  ...
  def create_auto_transaction
    if self.master_transaction_id == 0
      ...
      trigger.each do |t|
        ...
        FinanceTransaction.create(:title=> self.title + ' - ' + t.title.to_s ,:transaction_date=>self.transaction_date, :amount=>trigger_amount,:category_id =>self.category_id,:master_transaction_id=>self.id)
      end
    end
  end
  ...
  def self.total_transaction_amount(transaction_category,start_date,end_date)
    ...
    transactions = FinanceTransaction.where(["transaction_date >= ? AND transaction_date <= ? AND category_id = ?",start_date,end_date,finance_transaction_category.id])
