...
class FinanceTransactionCategory < ApplicationRecord
  ...
  scope :expense_categories, lambda{where("is_income = false AND name NOT LIKE 'Salary'and deleted = 0")}
  ...
  def total_income(start_date,end_date)
    if is_income
      self.finance_transactions.where(["transaction_date >= ? AND transaction_date <= ? AND master_transaction_id=0",start_date,end_date]).map{|ft| ft.amount}.sum
    else
      0
    end
  end
  ...
  def total_expense(start_date,end_date)
    if is_income
      ...
      self.finance_transactions.where(["transaction_date >= ? AND transaction_date <= ?",start_date,end_date]).map{|ft| ft.amount}.sum
    end
  end
