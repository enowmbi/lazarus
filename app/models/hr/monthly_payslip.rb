...
class MonthlyPayslip < ApplicationRecord
  ...
  def self.find_and_filter_by_department(salary_date,dept_id)
    ...
    unless dept_id =="All"
      ...
      payslips = self.includes(:payroll_category).where(["salary_date = ? AND employee_id IN (?)",salary_date.to_date,all_employees_in_dept]).order("payroll_category_id ASC")
      individual_payslip_category = IndividualPayslipCategory.where(["salary_date = ? AND employee_id IN (?)",salary_date.to_date,all_employees_in_dept]).order("id ASC")
    else
      payslips = self.includes(:payroll_category).where("salary_date = ?",salary_date.to_date).order("payroll_category_id ASC")
  ...
  def active_or_archived_employee
    ...
    if employee.blank?
      return  ArchivedEmployee.find("former_id= ?",self.employee_id).first
    else
      return employee
    end
  end
  ...
  def self.total_employees_salary(start_date,end_date,dept_id="")
    ...
    unless dept_id.blank?
      ...
      archived_employees_in_dept = ArchivedEmployee.select("former_id").where("employee_department_id = ?",dept_id)
      all_employees_in_dept = active_employees_in_dept.collect(&:id) + archived_employees_in_dept.collect{|a| a.former_id.to_i}
      total_monthly_payslips = self.includes(:payroll_category).select("employee_id,amount,payroll_category_id,salary_date").where(["salary_date >= ? and salary_date <= ? and is_approved = 1 and employee_id IN (?)",start_date.to_date,end_date.to_date,all_employees_in_dept]).order("salary_date desc")
    else
      total_monthly_payslips = self.includes(:payroll_category).select("employee_id,amount,payroll_category_id,salary_date").where(["salary_date >= ? and salary_date <= ? and is_approved = 1",start_date.to_date,end_date.to_date]).order("salary_date desc")
    end
    ...
    unless employee_ids.blank?
      employee_ids.uniq!
      total_individual_payslips = IndividualPayslipCategory.where("salary_date >=  ? AND salary_date <= ? AND employee_id IN (#{employee_ids.join(",")})",start_date.to_date,end_date.to_date).order("id ASC")
    end
    total_salary = 0
