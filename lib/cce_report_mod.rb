...
module CceReportMod
  ...
  module InstanceMethods
    ...
    def elective_subjects
      subjects.all.where({:batch_id=>batch_in_context_id})
    end
    ...
    def subject_fa_scores
      hsh = {}
      cce_reports.scholastic.all.where(["batch_id=?",batch_in_context_id]).select("cce_reports.*,exams.subject_id,fa_criterias.fa_group_id").join('INNER JOIN fa_criterias ON cce_reports.observable_id = fa_criterias.id INNER JOIN exams ON exams.id = cce_reports.exam_id').group(&:subject_id).each do |key,val|
        hsh[key.to_i]={}
    ...
    def make_scholastic_report
      ...
      all_weightages = CceWeightage.all.join(:courses).where({:courses=>{:id=>batch_in_context.course_id}})
      sub_fa_scores.each{|k1,v1| v1.each{|k2, v2| fg_ids<<v2.keys}; exam_ids << v1.keys}
      examscores = exam_scores.all.join({:exam=>:exam_group}).where({:exam_id=>exam_ids.flatten.uniq,:exam_groups=>{:batch_id=>batch_in_context_id}}).include({:exam=>:exam_group})
      fgs= FaGroup.where(["id =?",fg_ids.flatten.uniq])
      s_arr = []
    ...
    def coscholastic_scores
      hsh={}
      cce_reports.coscholastic.select("cce_reports.*,observations.observation_group_id,observations.name AS o_name, observations.sort_order ").joins('INNER JOIN observations ON cce_reports.observable_id = observations.id').where(["batch_id=?", batch_in_context_id]).order("observations.sort_order ASC").group(&:observation_group_id).each do |key,val|
        hsh[key.to_i]={}
