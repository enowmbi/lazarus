...
class PayslipTransactionJob
  ...
  def perform
    @employee_ids.each do |employee_id|
      ...
      monthly_payslips = MonthlyPayslip.where(["employee_id=? AND salary_date = ?", employee.id, @salary_date]).includes(:payroll_category)
      individual_payslips =  IndividualPayslipCategory.where(["employee_id=? AND salary_date = ?", employee.id, @salary_date])
